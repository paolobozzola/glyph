...
├─ GlyphQuickLookPreview/      # (M3)
├─ GlyphQuickLookThumbnail/    # (M3)
...
## 3. Document model  *(AppKit `NSDocument` — actual M1 implementation)*

> Pivoted from SwiftUI `ReferenceFileDocument` at M1: SwiftUI couples a document's
...
- **M0 — Bridge spike. ✅ DONE.** App loads Milkdown in a `WKWebView`; edits posted back to
  Swift and logged. Builds & runs (Xcode 26.3).
- **M1 — Real documents. ✅ DONE.** Pivoted to AppKit **`NSDocument`** (decouples dirty from
